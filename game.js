...
{
    var t = GetType(piece);
    var scores = [0.0, 1.0, 3.0, 3.0, 4.0, 7.0, 5.0];
...
function RateMove(py, px, move, temp_board)
{
    var score = 0.1;

    var prev = temp_board[move.row][move.col];
    temp_board[move.row][move.col] = move.piece;
    temp_board[py][px] = NO_PIECE;

    var is_check = CheckForCheck(WHITE_PLAYER, temp_board);

    temp_board[py][px] = move.piece;
    temp_board[move.row][move.col] = prev;

    if (is_check) score += 200.0;
    if (move.cap) score += 10.0 * PieceValue(prev);
...
function FindBestMove(pieces)
{
    ...
    for (var i = 0; i < 8; i++)
    {
        temp_board.push([]);
        for (var j = 0; j < 8; j++)
        {   temp_board[i].push(board_data[i][j]);  }
    }

    for (var p in pieces) 
    {
        for (var m in pieces[p].pmoves)
        {
            var r = RateMove(pieces[p].py, pieces[p].px, pieces[p].pmoves[m],
                            temp_board);
            total_rating += r;
            rankings.push(r);
    ...
    var selection = Math.floor(Math.random() * total_rating);
    var r = 0.0; var i = 0;
    while (r < selection && i < moves.length) 
    {
        r += rankings[i];
        if (r < selection) i++;
    }
...
function ComputersTurn() 
{
    ...
    if (pieces.length) {
        ...
        var cmp_move = FindBestMove(pieces);
        //var piece = cmp_move.piece;
        //var piece = Math.floor(Math.random() * pieces.length);
        var piece = pieces[cmp_move.piece];
        game_mode = MODE_SELECT_PIECE;
        ClickBoard(piece.py, piece.px, true);
        computer_turn = [];
        computer_turn.pmoves = piece.pmoves;
        computer_turn.move_sel = cmp_move.move;
...
function ComputersTurnPart2()
{
    if (computer_turn.pmoves.length) {
        ...
        var move_sel = computer_turn.move_sel;
